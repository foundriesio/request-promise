 ...
 * @return {Promise}            Resolves to the hashed string.
 */
 ...
 */
async function getOrCreateHTTPAgent(agentOptions, httpOptions) {
    const key = await optionsToHash(agentOptions, httpOptions);
 ...
 * @return     {Object}  The `http.Agent` instance.
 */
async function getOrCreateHTTPSAgent(agentOptions, httpOptions) {
    const key = await optionsToHash(agentOptions, httpOptions, 'https');
 ...
 */
RequestAgent.prototype.update = async function(oldURL, newURL) {
    ...
    if ((oldURL.protocol !== newURL.protocol) ||
        ...
        if (newURL.protocol.startsWith('https')) {
            if (this.newAgent) {
                ...
                this.httpOptions.agent = await getOrCreateHTTPSAgent(
            ...
            if (this.newAgent) {
                ...
                this.httpOptions.agent = await getOrCreateHTTPAgent(
                    this.agentOptions, this.httpOptions);
            }
...
RequestAgent.prototype.setHTTPOptions = function(uri, options = {}) {
    ...
    this.httpOptions.port = reqURL.port;
 ...
 */
async function create(uri, options = {}) {
    if (!isObject(options)) {
        return Promise.reject(
            new RequestAgentError('Passed options must be an object'));
    }
    ...
    if (uri.startsWith('http')) {
        ...
        if (agent.httpOptions.protocol.startsWith('https')) {
            if (!agent.newAgent) {
                agent.httpOptions.agent = await getOrCreateHTTPSAgent(
            ...
            if (!agent.newAgent) {
                agent.httpOptions.agent = await getOrCreateHTTPAgent(
                    agent.agentOptions, agent.httpOptions);
            }
        ...
        return Promise.reject(new RequestAgentError(`Unsupported protocol`));
    }
}
